...
def process_video(url, max_length, min_length):
    ...
    if not video_id:
        return (
            "Could not extract video ID from the provided URL. Please check the URL format.",
            "",
        )
    ...
    summary = summarizer.summarize(
        transcript, max_length=max_length, min_length=min_length
    )
    ...
    with gr.Row():
        ...
        with gr.Column(scale=3):
            ...
            transcript_output = gr.Textbox(
                label="Full Transcript", lines=10, interactive=False
            )
